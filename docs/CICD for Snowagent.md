...
based on a combination of configuration files and environment variables. We create a new file following the `config-$environment.yml` naming
convention, with content as presented in the conf/config-template.yml template. We also need to set environment variables to specify the
