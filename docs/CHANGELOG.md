...
- **Post-install verification bizevent**: a full deploy (`--scope=all`) or upgrade
  (`--scope=upgrade`) now ends by sending a `dsoa.installation` bizevent from inside Snowflake via
