...
- **OpenPipeline metric-extraction processors** (`docs/openpipeline/`): three new processors in the SnowAgent logs pipeline that derive counters directly from DSOA log telemetry without requiring custom queries or dashboards: `snowflake.task.run.failed`, `snowflake.task.run.cancelled`, and `snowflake.task.run.successful` (task run state counters from `task_history`, dimensioned by database/schema/task name). Deployed with `./scripts/deploy/deploy_dt_assets.sh --scope=openpipeline`. See `docs/openpipeline/README.md`.
