...
- **Timestamp Handling Standardization**: Refactored timestamp handling to use a unified approach with smart unit detection. All SQL views produce nanoseconds via `extract(epoch_nanosecond ...)`, which are converted to the appropriate unit (milliseconds or nanoseconds) only at the API boundary. This eliminates wasteful ns→ms→ns conversions and clarifies API requirements.
  - **Precision preservation**: `validate_timestamp()` now works internally in nanoseconds to avoid precision loss, converting to the requested unit only at the end
  - **Enhanced validation**: Added `skip_range_validation` parameter to preserve original timestamps without time range checking (used for `observed_timestamp`)
  - **Reusable utility**: Added `process_timestamps_for_telemetry()` utility function that implements the standard pattern for both logs and events. Hardcoded to always return timestamp in milliseconds and observed_timestamp in nanoseconds since these units are consistent across all use cases:
    - Gets timestamp and observed_timestamp from data
    - Falls back to timestamp value when observed_timestamp is not provided
    - Validates timestamp with range checking, observed_timestamp without range checking
    - Returns (timestamp_ms, observed_timestamp_ns) tuple
  - **Consistent handling**: Logs and Events now use the same timestamp processing logic via shared utility
  - **Fallback logic**: `observed_timestamp` now correctly falls back to `timestamp` value when not explicitly provided by plugins (only event_log plugin provides it)
  - **Strict parameter validation**: `validate_timestamp()` now raises `ValueError` if `return_unit` is neither "ms" nor "ns"
  - `observed_timestamp` now correctly uses nanoseconds for all telemetry types (logs, events, spans, metrics) per OTLP standard
  - Added `validate_timestamp()` function with `return_unit` parameter for explicit control over output format
  - Added `get_timestamp()` function that returns nanoseconds from SQL queries
  - Removed obsolete internal functions `get_timestamp_in_ms()` and `validate_timestamp_ms()`
  - Added comprehensive API documentation links to all telemetry classes (Dynatrace and OTLP specs)
  - Note: Dynatrace OTLP Logs API requires milliseconds for `timestamp` field (deviates from OTLP standard)
- **Build System**: All `scripts/dev/` scripts now automatically activate the virtual environment (`.venv/`) to ensure consistent Python environment usage
- **Documentation**: Updated `.github/copilot-instructions.md` with comprehensive information about autogenerated files:
  - Documentation files: `docs/PLUGINS.md`, `docs/SEMANTICS.md`, `docs/APPENDIX.md` (generated from plugin source files)
  - Build artifacts: `build/_dtagent.py`, `build/_send_telemetry.py`, `build/_semantics.py`, `build/_version.py` (never commit these)
