...
- **Warehouse change attribution (experimental)** for `query_history` plugin: new `track_ddl_changes` config key (default `false`) surfaces `snowflake.object.{type,id,name}`, `snowflake.object.ddl.operation`, and `snowflake.object.ddl.properties` on `query_history` events whose `ACCESS_HISTORY.OBJECT_MODIFIED_BY_DDL` payload is populated. Warehouse and resource-monitor DDL rows are held back until `ACCESS_HISTORY` catches up (~3 h lag) so one enriched event is emitted instead of two. Ships with a new `Warehouse Change Detection` dashboard and `warehouse-sensitive-change-alert` workflow. May be refactored into a dedicated plugin in a future release. See [devlog](../.context/devlog/unreleased/warehouse-change-attribution.md) for design rationale.
- **OpenPipeline metric-extraction processors** (`docs/openpipeline/`): six processors in the SnowAgent logs pipeline that derive counters directly from DSOA log telemetry without requiring custom queries or dashboards. Operations: `snowflake.task.run.failed`, `snowflake.task.run.cancelled`, `snowflake.task.run.successful` (task run state counters from `task_history`, dimensioned by database/schema/task name). Security: `snowflake.login.attempts.failed`, `snowflake.login.attempts.successful`, `snowflake.login.attempts.total` (login counters from `login_history`, dimensioned by db.system/environment/user/host/client type). Deployed with `./scripts/deploy/deploy_dt_assets.sh --scope=openpipeline`. See `docs/openpipeline/README.md`.
- **Event Log: per-database event table support** (`discover_db_event_tables: false` opt-in). Discovers Snowflake DB-level `EVENT_TABLE` parameter overrides, unions them with the account-level source into `STATUS.EVENT_LOG`, and tags each record with `_dsoa_source_table` in `_RESOURCE_ATTRIBUTES`. Prevents silent telemetry blackout for multi-tenant customers using DB-scoped event table isolation. Opt-in default preserves backward compatibility for existing deployments. ([BDX-716])
- **Query text obfuscation** for `query_history` plugin: new `obfuscation_mode` config key (`off` / `literals` / `full`) controls whether query text and error messages are redacted before reaching Dynatrace. Mode `literals` replaces string/numeric literals with `?` while preserving SQL structure; mode `full` replaces the entire text with `[OBFUSCATED]`. Applies to both `db.query.text` and `snowflake.error.message`. Default `off` preserves backward compatibility. See [devlog](../.context/devlog/unreleased/query-text-obfuscation.md) for implementation details.
...
- Cross-batch span parent linking for `query_history` plugin: OTEL span context (`OTEL_SPAN_ID`, `OTEL_TRACE_ID`) is now persisted in `PROCESSED_QUERIES_CACHE` and used to inject parent context for child queries whose parent was processed in a previous batch. Enables continuous trace chains across agent run cycles. Cache TTL is configurable via `query_history.cache_ttl_hours` (default: 4h). See [devlog](../.context/devlog/0.9.5/cross-batch-span-linking.md) for implementation details.
- Cold tables identification plugin: identifies tables with no recent query access (default: >90 days) to enable FinOps teams to find candidates for archiving, dropping, or tiering to lower-cost storage. Reduces storage costs by sunsetting unused tables. See [Cold Tables plugin](PLUGINS.md#cold_tables_info_sec).
...
- **Snowflake Consumption (Organization Level) dashboard** (Phase A): overhauled `Org-Level Costs Observability` dashboard with new title, three sections (§2 Credit Consumption, §4 Storage, §5 Data Transfer), corrected Remaining Contract Balance tile using real metric names, and three variables (`$Accounts`, `$credit_rate`, `$bu_mapping`). See [devlog](../.context/devlog/unreleased/consumption-dashboard-phase-a.md).
- **Snowflake Consumption dashboard** (Phase C — §7 Department / BU View): adds five tiles grouping credits, USD billing, and storage by account with a `bu` field seeded from the hidden `$bu_mapping` variable. In v1 the BU field defaults to `"Unassigned"` for all accounts; configure exact account-to-BU mappings via the `$bu_mapping` JSON variable in the Dynatrace UI. Pattern-based mapping is planned for a future release. See [devlog](../.context/devlog/unreleased/consumption-dashboard-phase-c.md).
- **Snowflake Consumption (Organization Level) dashboard** (Phase B): added §1 Contract Capacity KPIs (6 tiles: capacity used, remaining capacity, 30-day run rate, YoY burn rate comparison, estimated days to overage, projected overage date) and §3 USD Consumption (3 tiles). Fixed `Org Contract Balance Warning` workflow to use correct metric names. See [devlog](../.context/devlog/unreleased/consumption-dashboard-phase-b.md).
...
- Updated `snowflake-snowpark-python` minimum version to `>=1.49.0` (was `>=1.48.1`). Python version constraint
  remains `<3.14` — bottleneck is `snowflake==1.12.0`, not snowpark. See [devlog](../.context/devlog/0.9.5/snowflake-sdk-audit.md) for full audit details.
- Improved memory handling and processing performance for high-volume Snowflake accounts. The hot-path
  ...
  run for memory self-monitoring. See [devlog](../.context/devlog/0.9.5/performance-memory-handling.md) for full technical details.
...
- **Admin deployment ordering**: `scope=admin` now correctly overwrites non-admin procedure stubs. Previously `10_admin.sql` ran before `30_plugins/`, causing admin overrides to be silently clobbered by non-admin stubs. The fix moves admin assembly to `80_admin.sql`, which sorts after all plugin files. Plugin developers using Pattern B (two-file: stub in plugin scope, override in `admin/` directory) no longer need inline `--%OPTION:dtagent_admin:` blocks in non-admin files. See [devlog](../.context/devlog/0.9.5/admin-deployment-order-fix.md) for details. (BIZOBS-115)
...
- `serverless_tasks` context: `db.namespace` and `snowflake.schema.name` no longer emit empty strings for DSOA's
  ...
  them from customer serverless tasks. See [devlog](../.context/devlog/unreleased/serverless-tasks-namespace-fix.md) for details. (BDX-1904 / TI-003)
...
See [devlog](../.context/devlog/unreleased/) for implementation details.
