...
| Configuration Key       | Type    | Default  | Description                                                                                                                                                                                                                   |
| ----------------------- | ------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lookback_hours`        | Integer | varies   | How far back (in hours) the plugin looks for new data on each run. **Only applies when no prior processed timestamp exists** (first run, or after a reset). During normal operation the plugin advances from the last processed timestamp automatically. See each plugin's `config.md` for the default value and any additional per-context lookback keys (e.g., `lookback_hours_versions` for the `tasks` plugin). |
