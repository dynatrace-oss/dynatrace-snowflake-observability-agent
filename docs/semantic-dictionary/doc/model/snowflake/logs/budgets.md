<!-- model snowflake.logs.budgets -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake budgets log records

Log records emitted by the DSOA budgets plugin.

### Query

Fetch the 100 most recent DSOA budgets plugin log entries from Grail.

*Davis CoPilot description: Search for all budgets plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Show recent Snowflake budget spend and limit values per budget.*
...
*Davis CoPilot description: Find which Snowflake budgets have spent the most credits.*
...
Snowflake budgets log records implements the following interfaces: *i.dsoa_resource*
