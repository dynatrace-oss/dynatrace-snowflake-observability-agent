<!-- model snowflake.logs.login_history -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake login history log records

Log records emitted by the DSOA login_history plugin.

### Query

Fetch the 100 most recent DSOA login_history plugin log entries from Grail.

*Davis CoPilot description: Search for all login_history plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Find failed Snowflake login attempts with error codes and authentication type.*
...
*Davis CoPilot description: Find which source IPs and users generate the most Snowflake login attempts.*
...
Snowflake login history log records implements the following interfaces: *i.dsoa_resource*
