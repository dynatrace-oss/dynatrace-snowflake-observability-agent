<!-- model snowflake.logs.users -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake users log records

Log records emitted by the DSOA users plugin.

### Query

Fetch the 100 most recent DSOA users plugin log entries from Grail.

*Davis CoPilot description: Search for all users plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Search for disabled Snowflake user records in the users plugin logs.*
...
*Davis CoPilot description: Search for Snowflake users that do not have MFA configured.*
...
Snowflake users log records implements the following interfaces: *i.dsoa_resource*
