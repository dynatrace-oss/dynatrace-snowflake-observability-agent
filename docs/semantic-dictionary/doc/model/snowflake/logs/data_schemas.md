<!-- model snowflake.logs.data_schemas -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake data schemas log records

Log records emitted by the DSOA data_schemas plugin.

### Query

Fetch the 100 most recent DSOA data_schemas plugin log entries (DDL change inventory) from Grail.

*Davis CoPilot description: Search for all data_schemas plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Find recent DROP DDL operations on Snowflake objects.*
...
*Davis CoPilot description: Find recent table structure changes captured by the data_schemas plugin.*
...
Snowflake data schemas log records implements the following interfaces: *i.dsoa_resource*
