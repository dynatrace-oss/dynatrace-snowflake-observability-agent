<!-- model snowflake.logs.tasks -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake tasks log records

Log records emitted by the DSOA tasks plugin.

### Query

Fetch the 100 most recent DSOA tasks plugin log entries from Grail.

*Davis CoPilot description: Search for all tasks plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Search for failed Snowflake task runs in the tasks plugin logs.*
...
*Davis CoPilot description: Find which Snowflake tasks run most frequently.*
...
Snowflake tasks log records implements the following interfaces: *i.dsoa_resource*
