<!-- model snowflake.logs.query_history -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake query history log records

Log records emitted by the DSOA query_history plugin.

### Query

Fetch the 100 most recent DSOA query_history plugin log entries from Grail.

*Davis CoPilot description: Search for all query_history plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Find long-running Snowflake queries exceeding five minutes of total elapsed time.*
...
*Davis CoPilot description: Find which Snowflake warehouses have the slowest queries by p90 execution time.*
...
Snowflake query history log records implements the following interfaces: *i.dsoa_resource*
