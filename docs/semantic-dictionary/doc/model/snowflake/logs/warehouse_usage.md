<!-- model snowflake.logs.warehouse_usage -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake warehouse usage log records

Log records emitted by the DSOA warehouse_usage plugin.

### Query

Fetch the 100 most recent DSOA warehouse_usage plugin log entries from Grail.

*Davis CoPilot description: Search for all warehouse_usage plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Show Snowflake warehouse start, resume and suspend events.*
...
*Davis CoPilot description: Break down Snowflake warehouse state-change events by warehouse and event type.*
...
Snowflake warehouse usage log records implements the following interfaces: *i.dsoa_resource*
