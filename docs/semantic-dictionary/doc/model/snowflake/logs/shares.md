<!-- model snowflake.logs.shares -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake shares log records

Log records emitted by the DSOA shares plugin.

### Query

Fetch the 100 most recent DSOA shares plugin log entries (inbound and outbound shares) from Grail.

*Davis CoPilot description: Search for all shares plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Show Snowflake outbound shares and the tables they expose.*
...
*Davis CoPilot description: Find which Snowflake shares expose the most objects.*
...
Snowflake shares log records implements the following interfaces: *i.dsoa_resource*
