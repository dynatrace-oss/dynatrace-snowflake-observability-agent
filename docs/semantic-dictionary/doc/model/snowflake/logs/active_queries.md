<!-- model snowflake.logs.active_queries -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake active queries log records

Log records emitted by the DSOA active_queries plugin.

### Query

Fetch the 100 most recent DSOA active_queries plugin log entries from Grail.

*Davis CoPilot description: Search for all active_queries plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Find failed Snowflake query executions reported by the active_queries plugin.*
...
*Davis CoPilot description: Find which Snowflake warehouses have the slowest currently running queries by average running time.*
...
Snowflake active queries log records implements the following interfaces: *i.dsoa_resource*
