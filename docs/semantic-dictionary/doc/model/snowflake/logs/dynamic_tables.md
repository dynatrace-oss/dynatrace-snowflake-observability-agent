<!-- model snowflake.logs.dynamic_tables -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake dynamic tables log records

Log records emitted by the DSOA dynamic_tables plugin.

### Query

Fetch the 100 most recent DSOA dynamic_tables plugin log entries from Grail.

*Davis CoPilot description: Search for all dynamic_tables plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Find Snowflake dynamic tables that failed to meet their target refresh lag.*
...
*Davis CoPilot description: Find the Snowflake dynamic tables with the highest refresh lag.*
...
Snowflake dynamic tables log records implements the following interfaces: *i.dsoa_resource*
