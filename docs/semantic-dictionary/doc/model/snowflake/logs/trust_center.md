<!-- model snowflake.logs.trust_center -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake trust center log records

Log records emitted by the DSOA trust_center plugin.

### Query

Fetch the 100 most recent DSOA trust_center plugin log entries from Grail.

*Davis CoPilot description: Search for all trust_center plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Find critical Snowflake Trust Center security findings.*
...
*Davis CoPilot description: Summarise Snowflake Trust Center findings by scanner type and risk level.*
...
Snowflake trust center log records implements the following interfaces: *i.dsoa_resource*
