<!-- model snowflake.logs.snowpipes -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake snowpipes log records

Log records emitted by the DSOA snowpipes plugin.

### Query

Fetch the 100 most recent DSOA snowpipes plugin log entries from Grail.

*Davis CoPilot description: Search for all snowpipes plugin logs from the Snowflake observability agent.*
...
*Davis CoPilot description: Find Snowflake pipes reporting ingestion errors.*
...
*Davis CoPilot description: Find which Snowflake pipes have loaded the most rows.*
...
Snowflake snowpipes log records implements the following interfaces: *i.dsoa_resource*
