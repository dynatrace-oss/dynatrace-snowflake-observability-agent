<!-- model snowflake.spans.query_history -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake query history spans

Span records emitted by the DSOA query_history plugin.

### Query

Fetch the 50 most recent DSOA query_history spans (individual Snowflake query executions).

*Davis CoPilot description: Find query_history span traces produced by the Snowflake observability agent.*
...
*Davis CoPilot description: Find the trace and span identifiers for Snowflake query execution spans, including whether each is the root span.*
...
*Davis CoPilot description: List query execution plan steps and operators recorded as span events on Snowflake query spans.*
...
Snowflake query history spans implements the following interfaces: *i.dsoa_resource*
