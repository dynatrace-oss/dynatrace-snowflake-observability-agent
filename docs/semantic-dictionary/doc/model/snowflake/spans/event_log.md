<!-- model snowflake.spans.event_log -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake event log spans

Span records emitted by the DSOA event_log plugin.

### Query

Fetch the 50 most recent DSOA event_log spans (traced records from the Snowflake event table).

*Davis CoPilot description: Find event_log span traces produced by the Snowflake observability agent.*
...
*Davis CoPilot description: Show event_log spans linked to Snowflake queries with their warehouse and role.*
...
*Davis CoPilot description: Break down event_log spans by Snowflake warehouse.*
...
Snowflake event log spans implements the following interfaces: *i.dsoa_resource*
