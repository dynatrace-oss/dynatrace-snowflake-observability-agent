<!-- model snowflake.events.shares -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake shares lifecycle events

Timestamp-based state-change events emitted by the DSOA shares plugin via the OpenPipeline Events API.

### Query

Fetch recent DSOA self-monitoring business events for the shares plugin.

*Davis CoPilot description: Search for self-monitoring events from the shares Snowflake agent plugin.*
...
*Davis CoPilot description: Search for Snowflake share and grant lifecycle events from the shares plugin.*
...
*Davis CoPilot description: Break down Snowflake share lifecycle events by their event type.*
...
Snowflake shares lifecycle events implements the following interfaces: *i.dsoa_resource*
