<!-- model snowflake.events.tasks -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake tasks lifecycle events

Timestamp-based state-change events emitted by the DSOA tasks plugin via the OpenPipeline Events API.

### Query

Fetch the most recent DSOA tasks lifecycle events (task graph version anchors).

*Davis CoPilot description: Search for Snowflake task graph lifecycle events from the tasks plugin.*
...
*Davis CoPilot description: Break down Snowflake task lifecycle events by their event type.*
...
*Davis CoPilot description: Show Snowflake task lifecycle events with task name and event type.*
...
Snowflake tasks lifecycle events implements the following interfaces: *i.dsoa_resource*
