<!-- model snowflake.events.data_volume -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake data volume lifecycle events

Timestamp-based state-change events emitted by the DSOA data_volume plugin via the OpenPipeline Events API.

### Query

Fetch recent DSOA self-monitoring business events for the data_volume plugin.

*Davis CoPilot description: Search for self-monitoring events from the data_volume Snowflake agent plugin.*
...
*Davis CoPilot description: Search for Snowflake table DDL and data-update lifecycle events from the data_volume plugin.*
...
*Davis CoPilot description: Break down Snowflake data_volume lifecycle events by their event type.*
...
Snowflake data volume lifecycle events implements the following interfaces: *i.dsoa_resource*
