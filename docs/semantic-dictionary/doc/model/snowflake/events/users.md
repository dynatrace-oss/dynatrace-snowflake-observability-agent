<!-- model snowflake.events.users -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake users lifecycle events

Timestamp-based state-change events emitted by the DSOA users plugin via the OpenPipeline Events API.

### Query

Fetch recent DSOA self-monitoring business events for the users plugin.

*Davis CoPilot description: Search for self-monitoring events from the users Snowflake agent plugin.*
...
*Davis CoPilot description: Search for Snowflake user lifecycle events from the users plugin.*
...
*Davis CoPilot description: Break down Snowflake user lifecycle events by their event type.*
...
Snowflake users lifecycle events implements the following interfaces: *i.dsoa_resource*
