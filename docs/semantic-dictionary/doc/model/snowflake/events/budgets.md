<!-- model snowflake.events.budgets -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake budgets lifecycle events

Timestamp-based state-change events emitted by the DSOA budgets plugin via the OpenPipeline Events API.

### Query

Fetch the most recent DSOA budgets lifecycle events from Grail.

*Davis CoPilot description: Search for Snowflake budget lifecycle events emitted by the observability agent.*
...
*Davis CoPilot description: Break down Snowflake budget lifecycle events by their event type.*
...
*Davis CoPilot description: Find when Snowflake budgets were created from the budgets plugin lifecycle events.*
...
Snowflake budgets lifecycle events implements the following interfaces: *i.dsoa_resource*
