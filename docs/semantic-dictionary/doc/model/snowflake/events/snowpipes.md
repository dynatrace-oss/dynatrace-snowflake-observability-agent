<!-- model snowflake.events.snowpipes -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake snowpipes lifecycle events

Timestamp-based state-change events emitted by the DSOA snowpipes plugin via the OpenPipeline Events API.

### Query

Fetch recent DSOA self-monitoring business events for the snowpipes plugin.

*Davis CoPilot description: Search for self-monitoring events from the snowpipes Snowflake agent plugin.*
...
*Davis CoPilot description: Search for Snowflake pipe lifecycle events from the snowpipes plugin.*
...
*Davis CoPilot description: Break down Snowflake pipe lifecycle events by their event type.*
...
Snowflake snowpipes lifecycle events implements the following interfaces: *i.dsoa_resource*
