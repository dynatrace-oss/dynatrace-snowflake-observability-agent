<!-- model snowflake.events.dynamic_tables -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake dynamic tables lifecycle events

Timestamp-based state-change events emitted by the DSOA dynamic_tables plugin via the OpenPipeline Events API.

### Query

Fetch the most recent DSOA dynamic_tables lifecycle events (scheduling suspend/resume, graph version).

*Davis CoPilot description: Search for Snowflake dynamic table scheduling and graph lifecycle events.*
...
*Davis CoPilot description: Break down Snowflake dynamic table lifecycle events by their event type.*
...
*Davis CoPilot description: Find when Snowflake dynamic table scheduling was suspended.*
...
Snowflake dynamic tables lifecycle events implements the following interfaces: *i.dsoa_resource*
