<!-- model snowflake.events.resource_monitors -->
<!-- The content between the markdown start and end comments (tags) is generated. Please do not edit manually. -->
## Snowflake resource monitors lifecycle events

Timestamp-based state-change events emitted by the DSOA resource_monitors plugin via the OpenPipeline Events API.

### Query

Fetch the most recent DSOA resource_monitors lifecycle events (monitor and warehouse state changes).

*Davis CoPilot description: Search for Snowflake resource monitor and warehouse lifecycle events.*
...
*Davis CoPilot description: Break down Snowflake resource monitor lifecycle events by their event type.*
...
*Davis CoPilot description: Show Snowflake resource monitor lifecycle events with monitor name and event type.*
...
Snowflake resource monitors lifecycle events implements the following interfaces: *i.dsoa_resource*
