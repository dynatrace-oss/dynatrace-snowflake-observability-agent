...
| Warehouse change auditing             | Audit who changed which warehouse, when, and what SQL was executed. Tracks `ALTER`, `CREATE`, `DROP`, and `RENAME_WAREHOUSE` operations via `db.operation.name` on `query_history` logs.                 | [Query History plugin](PLUGINS.md#query_history_info_sec) |
